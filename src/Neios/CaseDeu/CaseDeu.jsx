...
const CaseDeu = () => {
  ...
  // Filter out the case study matching current route
  const visibleStudies = caseStudies
    .filter(study => study.path !== pathname)
    .slice(0, visibleCount);

  return (
    <div className="w-full h-auto bg-[#0C1F0C] pt-12">
      <div className="pb-10 w-11/12 lg:w-10/12 mx-auto">
        {/* Heading */}
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          {visibleStudies.map((study, index) => (
            <div
              ...
              className={`group w-full pt-3 lg:py-5 lg:px-0 px-2 pb-10 h-auto lg:h-[650px] xl:h-[720px] 2xl:h-[870px] relative lg:hover:-rotate-3 duration-300 rounded-[32px] ${getMarginTopClass(index)}`}
            >
              {/* Background */}
                          ...
                          <div className="relative z-50 px-[15px] py-[7px] lg:px-6 ">
                            <div
                              ...
                              className="relative w-full    lg:rounded-3xl  aspect-[4/3] max-h-[600px]  rounded-xl overflow-hidden cursor-pointer"
                            >
                              <Image
                                src={study.image}
                                alt={study.title}
        ...
        {/* Show More Button */}
        {visibleCount < caseStudies.filter(study => study.path !== pathname).length && (
